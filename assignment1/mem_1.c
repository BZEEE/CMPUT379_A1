...
int main() {
    ...
    // allocate massive array for driver 1 program
    // see how memory regions change
    int *array = malloc(sizeof(int) * 10000000);
    for (int i = 0; i < 10000000; i++) {
        array[i] = i;
    }

    struct memregion *regions2 = (struct memregion *) malloc(size * sizeof(struct memregion));
